...
dinning_reccomendation_prompt = """You are a travel assistant. Given the **destination** and the **recommended places** to visit, suggest **dining options** that are popular or unique.
Make sure to return top resturant option for each place. Only provide a list of dining options, not the detailed itinerary."""
...
**duration**,**hotel links**, **recommended links**, **recommended_dinning** and **flights** create a **final travel itinerary**. The itinerary should be **day-by-day**, divided into **morning**, **afternoon**, and **evening**.
...
-> Match recommended_dinning with each day's itinerary and add it at end of each day.There can be multiple dinning options for each day.
-> You will also be provided with a **recommended link** which will be added to the end of the travel plan.
-> Add **Flights** after day by day itinerary, this will contain details of flight.There can be multiple flights details available, add all of them to the travel plan.
...
7. **Flights**: If flight details are available add it at the end of the travel plan. If it is empty or None, print no source provided.There can be multiple flight details available.


8. **Dining Options**: Add the dining options at the end of each day's itinerary. Match the dining options with the places the user plans to visit each day. A day can have multiple places to visit, so ensure the dining options are correctly matched.
...
Make sure to add flights details at the end of the travel plan.
"""
...
Flights: {flights}
Dinning recommendation: {dinning}
...
dinning_recommendation_user_prompt = """Recommend dining options based on the following details:
Destination: {Destination}
